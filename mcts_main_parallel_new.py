...
class SeachCollector:
    def __init__(self, cases):
        ...
        self.base_directory = os.path.join(
            os.path.abspath("logs_mcts"), "mcts-" + timestamp_value.strftime("%Y-%m-%d-%H-%M-%S") + "-" + name,
        )
    ...
    def save_mcts_data(self, env, root):
    ...
    def plot_mcts(self, env, root, iteration):
        ...
        while len(search_list) > 0:
            ...
            if len(current_node.q) == 0:
                node_name_label = f"Q: 0,  N: {current_node.n},  Grasp Q: {current_node.state.q_value:.3f}"
            else:
            ...
            (object_states, color_image, depth_image, new_image_q) = current_node.state.mcts_helper.simulation_recorder[
                current_node.state.uid
            ]
            ...
            #             (object_states, color_image, depth_image, new_image_q) = current_state.mcts_helper.simulation_recorder[key]
    ...
    custom_parameters = [
        {
            ...
            "type": str,
            "default": "ik",
            "help": "Controller to use for robot. Options are {ik, osc}",
        },
        {"name": "--num_envs", "type": int, "default": 1000, "help": "Number of environments to create"},
...
if __name__ == "__main__":
    ...
    for repeat_idx in range(repeat_num):
        ...
        while True:
            ...
            print(f"Max grasp Q value: {new_q_value}")

            # record
            collector.save_heightmaps(iteration, color_image, depth_image)
            # collector.save_heightmaps(iteration + 1000, color_image, depth_images[0])
            ...
            mcts_helper.simulation_recorder["root"] = (object_states[0], color_image, depth_image, new_q_value)
            root = PushSearchNode(initial_state)
            mcts = MonteCarloTreeSearch(root, env.num_envs - 1, args.time_limit)
            ...
            best_node = mcts.best_action_parallel(test)
            print("best node:")
            ...
            if not test:
                collector.save_mcts_data(env, root)
