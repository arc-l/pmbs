...
class PushState:
    ...
    def _move_result(self, move, restore_state=True):
        ...
        if key not in self.mcts_helper.simulation_recorder:
            ...
            if PushState.grasp_method == "classifier":
                color_images, depth_images, segm_images = self.mcts_helper.env.render_camera(
                    self.mcts_helper.other_id, color=True, depth=True, segm=True, focal_target=True
                )
                ...
                color_images, depth_images, segm_images = self.mcts_helper.env.render_camera(
                    self.mcts_helper.other_id, color=True, depth=True, segm=True, focal_target=False
                )
                color_image = color_images[0]
                depth_image = depth_images[0]
                segm_image = segm_images[0]
            ...
            # new_image_q = self.mcts_helper.grasp_eval(depth_image)
            # # new_image_q, _, _ = self.mcts_helper.get_grasp_q(color_image, depth_image, post_checking=True)
            self.mcts_helper.simulation_recorder[key] = object_states, color_image, depth_image, segm_image, new_image_q
        else:
            object_states, _, _, _, new_image_q = self.mcts_helper.simulation_recorder[key]
    ...
    def get_actions(self, color_image=None, segm_image=None):
        key = self.uid
        if key not in self.mcts_helper.move_recorder:
            ...
            if color_image is None:
                if key not in self.mcts_helper.simulation_recorder:
                    ...
                    _, color_image, _, segm_image, _ = self.mcts_helper.simulation_recorder[key]
            ...
            # )
            actions = sample_actions_parallel(color_image, segm_image, self.mcts_helper.pool, plot=False)
            moves = []
